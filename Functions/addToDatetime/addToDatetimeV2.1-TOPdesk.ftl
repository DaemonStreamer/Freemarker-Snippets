...
<#function ftlExtensionDatetimeToEpoch input=.now year=0 month=0 day=0 hour=0 minute=0 second=0>
	...
	<#local tm_min+=minute>
	<#local tm_sec+=second>
        <#local tm_offset=-3600000*(.now?string("Z")?number / 100)><#-- Offset of time_zone setting -->
        <#local td_offset=-86400000>
	<#local epoch = tm_sec*1000 + tm_min*60000 + tm_hour*3600000 + tm_yday*86400000 + (tm_year?number-70)*31536000000 + ((tm_year?number-69)/4)?floor*86400000 - ((tm_year?number-1)/100)?floor*86400000 + ((tm_year?number+299)/299)?floor*86400000 + tm_offset + td_offset >
